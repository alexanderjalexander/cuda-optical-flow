...
load2dSharedMemoryWithHalo(T *shared, T *global, int halo_radius, int widthGlobal, int heightGlobal)
{
    ...
    load2dSharedMemoryCore(shared, global, halo_radius, widthGlobal, heightGlobal, blockDim.x, tx, ty, xGlobal,
                           yGlobal);
}
 ...
 * corresponds to a data or halo element outside the original frame. Calls syncthreads at the end to ensure all threads
 * have finished loading their data before any subsequent operations.
 *
 * @param T The type of the data to be loaded.
 * @param shared  Device shared memory to load the data into, expected to point to the first element of a 2D array.
 * @param global Device global memory array containing input data.
                                  ...
                                  int heightGlobal)
{
    ...
    load2dSharedMemoryCore(shared, global, halo_radius, widthGlobal, heightGlobal, tileSize, tx, ty, xGlobal, yGlobal);
}
